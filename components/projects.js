				...
				<div className="flex flex-col justify-center content-center my-4">
					<div className="relative overflow-hidden pb-video-ratio">
						<iframe
							...
							src={`https://www.youtube-nocookie.com/embed/${ssrc}?rel=0&showinfo=0&autoplay=0&mute=1`}
							title={title}
							allowFullScreen
						/>
					</div>
						...
						{slides.length < 2 ? (
							...
							>
								{slides.map((slide) => (
									<div key={`project-${projectKey}-slide-${slide.key}`}>
										{" "}
										{/* This container div will be styled by react-slick carosuel. Do not style or add classes */}
										{slideContent(slide, title)}
									</div>
								))}
							</Slider>
